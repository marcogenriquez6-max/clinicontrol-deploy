      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', minHeight: '22cm' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14, fontSize: 10 }}>
          <div>
            ...
            {receta.diagnostico && <p style={{ margin: '0 0 3px' }}><strong>Diagnóstico:</strong> {receta.diagnostico}</p>}
...
export default function RecetasPage() {
  ...
  const [diagnosticoReceta, setDiagnosticoReceta] = useState<string>('');

  const nombrePaciente = (r: Receta) =>
    r.pacienteNombre || `${r.consulta?.paciente?.nombre || ''} ${r.consulta?.paciente?.apellido || ''}`.trim() || '—';
  const nombreMedico = (r: Receta) =>
    r.medicoNombre ? `Dr. ${r.medicoNombre}` : `Dr. ${r.consulta?.medico?.nombre || ''} ${r.consulta?.medico?.apellido || ''}`.trim();
  const datosImpresion = (r: Receta): RecetaPrintData => ({
    paciente: nombrePaciente(r),
    medico: nombreMedico(r),
    fecha: r.createdAt || new Date().toISOString(),
    diagnostico: diagnosticoReceta || undefined,
    instrucciones: r.instrucciones,
    medicamentos: (r.items || []).map((m) => ({ ...m, medNombre: m.medicamentoNombre || m.medicamento?.nombre })),
  });
  /** Abre el detalle y trae el diagnóstico CIE-10 de la consulta para imprimirlo en la receta. */
  const abrirDetalle = async (r: Receta) => {
    setSelectedReceta(r);
    setDiagnosticoReceta('');
    setShowModal(true);
    if (r.consultaId) {
      try {
        const c = (await consultaService.getById(r.consultaId)).data as unknown as { diagnosticos?: { cie10?: { codigo?: string; descripcion?: string }; codigoCie10?: string; descripcion?: string }[] };
        const dx = (c?.diagnosticos || [])
          .map((d) => `${d.cie10?.codigo || d.codigoCie10 || ''} ${d.cie10?.descripcion || d.descripcion || ''}`.trim())
          .filter(Boolean);
        setDiagnosticoReceta(dx.join(' · '));
      } catch { /* la receta se imprime sin diagnóstico */ }
    }
  };
        ...
        {(!r.items || r.items.length === 0) ? (
          ...
          <>
            {r.items.slice(0, 3).map((item: RecetaMedicamento, i: number) => (
              <Badge key={item.id || i} variant="neutral" className="text-[10px]">{item.medicamentoNombre || item.medicamento?.nombre || 'N/A'}</Badge>
            ))}
            {r.items.length > 3 && (
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center gap-3 pt-2">
              <Button variant="secondary" onClick={() => {
                reportesService.registrarImpresion('receta', `Receta #${selectedReceta.id} · ${nombrePaciente(selectedReceta)}`).catch(() => {});
                window.print();
              }}>
              ...
              <Button onClick={async () => {
                ...
                const { generarRecetaPdf } = await import('../utils/receta-pdf');
                await generarRecetaPdf(datosImpresion(selectedReceta));
                reportesService.registrarImpresion('receta-pdf', `Receta #${selectedReceta.id} · ${nombrePaciente(selectedReceta)}`).catch(() => {});
                toast('success', 'PDF generado', 'Receta descargada');
              }}>
            ...
            <div className="flex justify-end gap-3 pt-2 border-t border-[var(--border-primary)]">
              ...
              <Button variant="premium" onClick={async () => {
                ...
                if (previewReceta) {
                  toast('info', 'Generando PDF...');
                  const { generarRecetaPdf } = await import('../utils/receta-pdf');
                  await generarRecetaPdf(previewReceta);
                  toast('success', 'PDF generado', 'Receta descargada');
                }
              }}>
                <Printer className="w-4 h-4" />Emitir y Descargar PDF
              </Button>
            </div>
          </div>
        )}
